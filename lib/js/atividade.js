...
form.addEventListener('submit',(event)=>{
    ...
    formul.reset()
    // Reabilita o botão de envio após 1 segundos
    setTimeout(() => {
        tblIn()  
...
const btnAtvRot=document.querySelector("#btnAtvRot")
btnAtvRot.addEventListener("click",(evt)=>{
    evt.preventDefault()
    url= getRoot()+'controllers/controllerAtvRot'
    fetch(url)
    .then(res=>res.json()) 
    .then(res=>{
        tblIn()
